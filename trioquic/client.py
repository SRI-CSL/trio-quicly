...
async def open_quic_connection(
    ...
    This will open an IPv6 datagram socket to the specified QUIC server.

    TODO: Modeled after `trio.open_tcp_stream` for connecting to TCP endpoints and borrowing heavily from it.
      No, don't model after this as it uses socket.connect() or .bind(), which will fail for tests on localhost!
    ...
    print(f"socket targets for {format_host_port(host, port)}: {targets}")
    ...
    # TODO: Like `aioquic` for clients, use only first address and make it IPv6 if IPv4 returned...
    #   Then, we don't need to do Happy Eyeballs (RFC 6555) either!
    remote_address = targets[0][4]
    if len(remote_address) == 2:
        remote_address = ("::ffff:" + remote_address[0], remote_address[1], 0, 0)

    # explicitly enable IPv4/IPv6 dual stack
    client_socket = trio.socket.socket(socket.AF_INET6, socket.SOCK_DGRAM)
    client_socket.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_V6ONLY, 0)
    await client_socket.bind(("::", local_port, 0, 0))

    client = QuicClient(client_socket, remote_address, client_configuration)
    client._ensure_receive_loop()
    return client.connections.get(remote_address)
    ...
    #     else:
    #         client = QuicClient(winning_socket, winning_address, client_configuration)
    #         open_sockets.remove(winning_socket)
    #         return client.connections.get(winning_address)
